...
router.get('/dashboardSearch', (req, res) => {
    ...
    console.log(table);
    connection.query(`SELECT * FROM ${table} WHERE name LIKE ? OR img LIKE ?`, [`%${searchTerm}%`, `%${searchTerm}%`], (err, rows) => {
        if (err) {
            console.error('Error querying the database:', err);
            return res.status(500).send('Database query failed');
        }
...
router.get('/search', (req, res) => {
    ...
    connection.query(query, [`%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`], (err, rows) => {
        if (err) {
            console.error('Error querying the database:', err);
            return res.status(500).send('Database query failed');
        }
